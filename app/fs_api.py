...
class FsAPI:
    ...
    def xdg_open(self, path: str) -> dict:
        """
        Hand a file or directory to the desktop environment via xdg-open.
        Runs detached (Popen, not check_call) so the UI never blocks.
        ...
        try:
            subprocess.Popen(
                ...
                start_new_session=True,   # detach from Erebus process group
            )
            return {"ok": True}
        except FileNotFoundError:
            return {"ok": False, "error": "xdg-open not found — is xdg-utils installed?"}
        except Exception as e:
            return {"ok": False, "error": str(e)}
